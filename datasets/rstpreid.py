...
def pidStatistics(annos):
    group_dict = {}
    for anno in annos:
        curr_id = anno['id']
        tup = (curr_id, anno['img_path'])
        if curr_id not in group_dict:
            group_dict[curr_id] = [tup]
        else:
            group_dict[curr_id].append(tup)

    # 将分组后的字典值转换为列表
    grouped_list = list(group_dict.values())
    flag_len_max = []
    for i in grouped_list:
        if len(i) > len(flag_len_max):
            flag_len_max = i
    return len(flag_len_max)
...
class RSTPReid(BaseDataset):
    ...
    def __init__(self, root='', verbose=True):
        ...
        print(len(self.train_annos))
        pidstatistics = pidStatistics(self.train_annos)
        self.train, self.train_id_container = self._process_anno(self.train_annos, training=True,num_max=pidstatistics)
    ...
    def _process_anno(self, annos: List[dict], training=False,num_max=0):
        pid_container = set()
        if training:
            dataset = []
            image_id = 0
            for anno in annos:
                ...
                for caption in captions:
                    dataset.append((pid, image_id, img_path, caption,num_max))
                image_id += 1
