...
def process_matrix_chu2(tensor_matrix,scale):
    ...
    margin = max_value * 0.2  # 设置 margin
    threshold_min = min_value + margin
    threshold_max = max_value - margin

    mask_get_middle = (tensor_matrix < threshold_max) if scale[0]== 0 else (tensor_matrix > threshold_min) & (tensor_matrix < threshold_max)#
    ...
    filtered_matrix1[~mask_get_middle] =  0 if (scale[0]== 0 and scale[2]== 0) else (tensor_matrix[~mask_get_middle] / scale[1]) #(tensor_matrix[~mask_get_middle] / scale[1])#*scale[0]#*scale[2]
    filtered_matrix2[~mask_get_max] = tensor_matrix[~mask_get_max]
    filtered_matrix3[~mask_get_min] =  0 if (scale[0]== 0 and scale[2]== 0) else  (tensor_matrix[~mask_get_min] / scale[1]) #(tensor_matrix[~mask_get_min] / scale[1])#*scale[0]#*scale[2]
...
def compute_sdm_per(scores, pid, logit_scale, epsilon=1e-8):
    ...
    return loss+torch.tensor(np.random.uniform(0.001, 0.0001, (batch_size, 1))).squeeze(1).to(scores.device)


def compute_sdm_perx(scores, pid, logit_scale, epsilon=1e-8, tau=0.02, margin=0.1):
    """
     Similarity Distribution Matching
     """
    batch_size = scores.shape[0]
    ...
    alpha_i2t = ((scores / tau).exp() * labels / ((scores / tau).exp() * labels).sum(dim=1,
    ...
    GCL_loss = (-  (alpha_i2t * scores).sum(1) + tau * ((scores / tau).exp() * mask).sum(1).clamp(
...
def compute_GCL_per(scores, pid, tau, argc,scale,margin):
    batch_size = scores.shape[0]
    pid = pid.reshape((batch_size, 1))  # make sure pid size is [batch_size, 1]
    pid_dist = pid - pid.t()
    labels = (pid_dist == 0).float().cuda()
    ...
    loss_neg1 = (-  (alpha_i2t * scores).sum(1) + tau * ((scores_neg1 / tau).exp() * mask).sum(1).clamp(max=10e35).log() + argc[1]).clamp(min=0) \
                + (-  (alpha_t2i * scores.t()).sum(1) + tau * ((scores_neg1.t() / tau).exp() * mask).sum(1).clamp(max=10e35).log() + argc[1]).clamp(min=0)
    loss_neg2 = (-  (alpha_i2t * scores).sum(1) + tau * ((scores_neg2 / tau).exp() * mask).sum(1).clamp(max=10e35).log() + argc[2]).clamp(min=0) \
                + (-  (alpha_t2i * scores.t()).sum(1) + tau * ((scores_neg2.t() / tau).exp() * mask).sum(1).clamp(max=10e35).log() + argc[2]).clamp(min=0)
    loss_neg3 = (-  (alpha_i2t * scores).sum(1) + tau * ((scores_neg3 / tau).exp() * mask).sum(1).clamp(max=10e35).log() + argc[0]).clamp(min=0) \
                + (-  (alpha_t2i * scores.t()).sum(1) + tau * ((scores_neg3.t() / tau).exp() * mask).sum(1).clamp(max=10e35).log() + argc[0]).clamp(min=0)
...
def compute_rbs(i_feats, t_feats, i_tse_f, t_tse_f, pid, label_hat=None, tau=0.02, margin=0.1, loss_type='GCL',
                logit_scale=50,args=None):
    loss_bgm, _ = compute_per_loss(i_feats, t_feats, pid, tau, margin, loss_type, logit_scale,args)
    loss_tse, _ = compute_per_loss(i_tse_f, t_tse_f, pid, tau, margin, loss_type, logit_scale,args)
...
def compute_per_loss(image_features, text_features, pid, tau=0.02, margin=0.2, loss_type='GCL', logit_scale=50,args=None):
    # # normalized features
    image_norm = image_features / image_features.norm(dim=-1,
                                                      keepdim=True)  # image_features.norm(dim=-1, keepdim=True)相当于所有image_features元素的平方开根号
    text_norm = text_features / text_features.norm(dim=-1, keepdim=True)
    scores = text_norm @ image_norm.t()  # 交叉注意力相似度分数

    if 'GCL' in loss_type:
        per_loss = compute_GCL_per(scores, pid, tau, argc=args.argc,scale=args.scale,margin=margin)
