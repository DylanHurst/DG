...
from utils.options import get_args2
...
if __name__ == '__main__':
    ...
    if args.distributed:
        ...
        torch.distributed.init_process_cmdgroup(backend="nccl", init_method="env://")
        synchronize()
    device = "cuda"
    cur_time = time.strftime("%Y%m%d_%H%M%S", time.localtime())
    args.output_dir = op.join(args.output_dir, args.dataset_name, f'{cur_time}_{name}_{args.loss_names}')
    logger = setup_logger('DG', save_dir=args.output_dir, if_train=args.training, distributed_rank=get_rank())
    ...
    # tx1_train_loader=train_loader.detach()
